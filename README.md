...
| **Create All Campaign Types** | ✅ Live | Takes one campaign and builds the other three (🔵, Opt Out, 🔵 Opt Out) via `/campaign/duplicate` with sub-sequences, appends the opt-out spintax to step 1 of both Opt Out copies, sorts the not-contacted leads by mailbox provider and splits them four ways — Microsoft into the 🔵 campaigns, the rest halved between the original and its Opt Out copy — then activates all four via `/campaign/launch`. Background job with four-step progress; one job at a time; re-running adopts copies it already made. |
| **Create Follow Up Emails** | ✅ Live | Keeps a numbered library of follow-up templates (add/delete/bulk-paste, stored server-side per API key) and adds them all as variants of step 2 on any campaign — creating step 2 if the campaign only has step 1. The `SERVICE OFFERING / OFFER` placeholder is replaced with the offer sentence entered on the run. Preview before writing; re-running skips anything already there. |
| **General Bulk Actions** | ✅ Live | A container for actions that run across many workspaces at once, with a shared multi-select workspace picker. First action: **Add webhook** — creates the same webhook (`POST /hook/add`) in every selected workspace, skipping any that already point at the same URL. Preview before writing; each workspace reports its own outcome. |
...
| **Add Signatures** | ✅ Live | Generates hundreds of spintax signature combinations from your company / phone / address variations, personalizes each with the inbox's own name, and applies them across a workspace. Remembers each workspace's titles, company names, phones and addresses after a successful run, so adding signatures to that workspace's new inboxes only takes a click. |
