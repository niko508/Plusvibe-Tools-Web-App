...
| **Create All Campaign Types** | ✅ Live | Point it at the original campaign and it finds the three copies you duplicated in Plusvibe (🔵, Opt Out, 🔵 Opt Out) by name, appends the opt-out spintax to step 1 of both Opt Out campaigns, then sorts the not-contacted leads by mailbox provider and splits them four ways — Microsoft into the 🔵 campaigns, the rest halved between the original and its Opt Out copy. Runs as a background job with three-step progress; one job at a time. |
